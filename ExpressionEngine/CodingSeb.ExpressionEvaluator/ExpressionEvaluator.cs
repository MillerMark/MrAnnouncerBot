...
namespace CodingSeb.ExpressionEvaluator
{
	...
	public class TryStatement : CodePosition
	{
		...
		public TryStatement(string zero, string one, int i)
		{
			StartOffset = i - one.Length; // Not sure if this is going to be correct anymore.
			Zero = zero;
			One = one;
			Two = string.Empty;
		}

		public TryStatement(string zero, int i)
		{
			StartOffset = i - zero.Length; // Not sure if this is going to be correct anymore.
			Zero = zero;
			One = string.Empty;
			Two = string.Empty;
		}
	...
	public partial class ExpressionEvaluator
	{
		...
		public event ExecutionPointerChangedHandler ExecutionPointerChanged
		{
			add
			{
				if (executionPointerChanged == null)    // First listener...
				{
					anyoneListening = true;
				}
				executionPointerChanged = (ExecutionPointerChangedHandler)Delegate.Combine(executionPointerChanged, value);
			}
			remove
			{
				executionPointerChanged = (ExecutionPointerChangedHandler)Delegate.Remove(executionPointerChanged, value);
				if (executionPointerChanged == null)  // No more listeners to this event
				{
					anyoneListening = false;
				}
			}
		}
		...
		public virtual object ScriptEvaluate(string script)
		{
			...
			try
			{
				...
				ResetExecutionPointer(script);
				object result = ScriptEvaluate(script, ref isReturn, ref isBreak, ref isContinue, 0);
		...
		protected virtual object ScriptEvaluate(string script, ref bool valueReturned, ref bool breakCalled, ref bool continueCalled, int instructionPointerOffset)
		{
			...
			List<IfStatement> ifElseStatementsList = new List<IfStatement>();
			List<TryStatement> tryStatementsList = new List<TryStatement>();
			...
			void ExecuteIfList()
			{
				if (ifElseStatementsList.Count > 0)
				{
					...
					if (!string.IsNullOrEmpty(ifScript))
						lastResult = ScriptEvaluate(ifScript, ref isReturn, ref isBreak, ref isContinue, ifStatement.StartOffset);
			...
			void ExecuteTryList()
			{
				if (tryStatementsList.Count > 0)
				{
					...
					catch (Exception exception)
					{
						...
						foreach (TryStatement catchStatement in tryStatementsList.Skip(1).TakeWhile(e => e.Zero.Equals("catch")))
						{
							if (catchStatement.One != null)
							{
								string[] exceptionVariable = catchStatement.One.Trim().Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
			...
			while (!isReturn && !isBreak && !isContinue && i < script.Length)
			{
								...
								|| (blockKeywordsWithoutParenthesesBeginningMatch = blockKeywordsWithoutParenthesesBeginningRegex.Match(script.Substring(i))).Success))
				{
					...
					else if (keyword.Equals("finally", StringComparisonForCasing))
					{
						...
						{
							tryStatementsList.Add(new TryStatement("finally", subScript, i));
					...
					else
					{
						...
						{
							tryStatementsList.Add(new TryStatement(subScript, i));
							ifBlockEvaluatedState = IfBlockEvaluatedState.NoBlockEvaluated;
