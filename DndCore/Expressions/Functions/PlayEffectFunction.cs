...
namespace DndCore
{
	...
	public class PlayEffectFunction : DndFunction
	{
		...
		static void OnPlayKnownEffect(string effectName, string spellId, string taleSpireId, float lifeTime, EffectLocation effectLocation, float secondsDelayStart, float enlargeTime)
		{
			PlayEffect?.Invoke(null, new SpellEffectEventArgs(effectName, spellId, taleSpireId, effectLocation, lifeTime, secondsDelayStart, enlargeTime));
		}

		public override object Evaluate(List<string> args, ExpressionEvaluator evaluator, Creature player, Target target, CastedSpell spell, RollResults dice = null)
		{
			ExpectingArguments(args, 1, 5);
			...
			OnPlayKnownEffect(effectName, spell.ID, player.taleSpireId, lifeTime, effectLocation, secondsDelayStart, enlargeTime);
