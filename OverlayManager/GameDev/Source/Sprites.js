var Sprites = (function () {
    function Sprites(baseAnimationName, expectedFrameCount, frameInterval, animationStyle, onLoadedFunc) {
    ...
    Sprites.prototype.outlineRect = function (left, top, right, bottom, margin, startingSegment, numSegmentsToDraw) {
        var x;
        var y;
        this.sprites = [];
        var spriteCount = 0;
    ...
    Sprites.prototype.collect = function (left, top, width, height) {
        var margin = 10;
        left -= margin;
        top -= margin;
    ...
    Sprites.prototype.draw = function (context, now) {
        if (this.moves)
    ...
    Sprites.prototype.updatePositions = function (now) {
        this.sprites.forEach(function (sprite) {
            sprite.updatePosition(now);
        });
    };
    Sprites.prototype.bounce = function (left, top, right, bottom, now) {
        for (var i = this.sprites.length - 1; i >= 0; i--) {
            var sprite = this.sprites[i];
