class StatPage {
	...
	static createMainStatsPage(): StatPage {
		...
		statPage.addStat('tempHitPoints', hpTempHpX, 374, tempHpFontSize, 61);
		...
		statPage.addStat('SpellCastingStr', 248, 282, maxSpellFontSize, 69, TextAlign.left, TextDisplay.deemphasizeZero);
		statPage.addStat('maxHitPoints', 152, 282, maxSpellFontSize, 37, TextAlign.left, TextDisplay.autoSize);

		statPage.addStat('resistancesVulnerabilitiesImmunities', 202, 244, maxSpellFontSize, 202, TextAlign.left, TextDisplay.autoSize);
	...
	addStat(name: string, x: number, y: number, fontSize: number, maxWidth = 0, textAlign: TextAlign = TextAlign.center, textDisplay: TextDisplay = TextDisplay.normal): any {
		this.characterStats.push(new CharacterStat(name, x, y, fontSize, maxWidth, textAlign, textDisplay));
	}
}