...
class DieRoller {
	...
	getSkillSaveOrInitiativeModifier(diceRollData: DiceRollData, player: Character): number {
		if (player)
			...
			else if (diceRollData.type === DiceRollType.SavingThrow || diceRollData.type === DiceRollType.DamagePlusSavingThrow) {
	...
	addDieToMultiPlayerSummary(die: IDie, topNumber: number, cardDamageModsList: Map<number, Array<string>>, cardScoreModsList: Map<number, Array<string>>) {
		...
		let playerRoll: PlayerRoll = this.diceRollData.multiplayerSummary.find((value: PlayerRoll, index, obj) => value.name === die.playerName);
		if (!playerRoll)
			playerRoll = this.diceRollData.multiplayerSummary.find((value: PlayerRoll, index, obj) => value.id === die.creatureID);
		...
		if (playerRoll) {
			...
			if (die.creatureID < 0) {
				...
				if (logProgress) {
					console.log(`It's a player. Here's the die:`);
					console.log(die);
					console.log(`Here's the diceRollData...`);
					console.log(this.diceRollData);
				}
			...
			//console.log('scoreModifier: ' + scoreModifier);
			let critHit: boolean;
	...
	static readonly Nat20: number = 20;
	...
	getRollResults(tallyResults: boolean): RollResults {
		...
		let playerIdForTextMessages = Creature.invalidCreatureId;

		const initializeLocalArrays = (playerID: number) => {
			if (!totalScoringDice.has(playerID))
				totalScoringDice.set(playerID, 0);
		...
		const findNat20s = (playerID: number, die: IDie, topNumber: number) => {
			if (this.diceRollData.type === DiceRollType.WildMagicD20Check) {
				return;
			}

			if (!nat20s.get(playerID))
				nat20s.set(playerID, false);

			switch (die.rollType) {
				case DieCountsAs.savingThrow:
				case DieCountsAs.totalScore:
					if (topNumber === 20 && die.isD20)
						nat20s.set(playerID, true);
					break;
			}
		}
		...
		const tallyTotals = (playerID: number, die: IDie, topNumber: number) => {
			...
			if (logProgress) {
				console.log(`tallyTotals, Before - totalScores.get(${playerID}): ` + totalScoringDice.get(playerID));
			}
			...
			switch (die.rollType) {
				case DieCountsAs.savingThrow:
					...
					totalScoringDice.set(playerID, totalScoringDice.get(playerID) + topNumber);
					break;
				case DieCountsAs.totalScore:
					if (logProgress) {
					...
					if (this.diceRollData.type === DiceRollType.WildMagicD20Check) {
						if (topNumber === 1)
							totalScoringDice.set(playerID, 1);
						else if (!totalScoringDice.get(playerID))
							totalScoringDice.set(playerID, 0);
					}
					else {
						if (die.scoreMultiplier >= 0 || nat20s.get(playerID)) { // Negative scoreMultipliers cannot impact a nat20.
							totalScoringDice.set(playerID, totalScoringDice.get(playerID) + topNumber * die.scoreMultiplier);

							if (die.damageType === DamageType.Bless || die.damageType === DamageType.Bane) {
								blessBaneLuckMods.set(playerID, blessBaneLuckMods.get(playerID) + topNumber * die.scoreMultiplier);
							}
						}
					}
			...
			if (logProgress) {
				console.log(`tallyTotals, After - totalScores.get(${playerID}): ` + totalScoringDice.get(playerID));
			}
		}
		...
		for (let i = 0; i < this.dice.length; i++) {
			...
			findNat20s(playerID, die, topNumber);
	...
	bonusRollDealsDamage(damageStr: string, description = '', playerID = Creature.invalidCreatureId): void {
	...
	getFirstPlayerId() {
		let playerID = Creature.invalidCreatureId;
	...
	private countsAsTotalScore(die: IDie) {
		return die.inPlay && (die.rollType === DieCountsAs.totalScore || die.rollType === DieCountsAs.savingThrow);
	}
	...
	prepareBaseDie(diceGroup: DiceGroup, die: IDie, throwPower: number, xPositionModifier = 0) {
		...
		dieObject.quaternion.y = (Math.random() * 180 - 90) * Math.PI / 180;
		dieObject.quaternion.z = (Math.random() * 180 - 90) * Math.PI / 180;
		dieObject.quaternion.w = Math.random();
		//console.log('dieObject.quaternion.w: ' + dieObject.quaternion.w);
	...
	addInspirationParticles(die: IDie, playerID: number, hueShiftOffset: number, rotationDegeesPerSecond: number) {
		const sprite: SpriteProxy = diceLayer.addInspirationParticles(die, 960, 540, rotationDegeesPerSecond, diceLayer.getHueShift(playerID) + hueShiftOffset);
		die.attachedSprites.push(sprite);
		die.origins.push(diceLayer.inspirationParticles.getOrigin());
	}

	createDie(diceGroup: DiceGroup, quantity: number, numSides: number, damageType: DamageType, rollType: DieCountsAs, backgroundColor: string, textColor: string, throwPower = 1, xPositionModifier = 0, isMagic = false, creatureID = Creature.invalidCreatureId, scale = 1, dieType = '', hueVariancePercent = 0, modifier = 0, scoreMultiplier = 1): IDie[] {
		const allDice: IDie[] = [];
		...
		for (let i = 0; i < quantity; i++) {
			...
			if (die.rollType === DieCountsAs.inspiration) {
				...
				this.addInspirationParticles(die, creatureID, -hueShift, -rotation);
				const sprite: SpriteProxy = diceLayer.addInspirationSmoke(die, 960, 540, Math.floor(Math.random() * 360));
				die.attachedSprites.push(sprite);
				die.origins.push(diceLayer.inspirationSmoke.getOrigin());
			}
			if (isMagic) {
				const sprite: SpriteProxy = diceLayer.addMagicRing(die, 960, 540, die.scale, magicRingHueShift + Random.plusMinusBetween(10, 25));
				die.attachedSprites.push(sprite);
	...
	addDie(diceGroup: DiceGroup, dieStr: string, damageType: DamageType, rollType: DieCountsAs, backgroundColor: string, textColor: string, throwPower = 1, xPositionModifier = 0, isMagic = false, playerID = Creature.invalidCreatureId, dieType = '', hueVariancePercent = 0, scale = 1): IDie[] {
		...
		return this.createDie(diceGroup, quantity, numSides, damageType, rollType, backgroundColor, textColor, throwPower, xPositionModifier, isMagic, playerID, scale, dieType, hueVariancePercent, modifier, scoreMultiplier);
	}

	addDieFromStr(diceGroup: DiceGroup, playerID: number, diceStr: string, dieCountsAs: DieCountsAs, throwPower: number, xPositionModifier = 0, backgroundColor: string = undefined, fontColor: string = undefined, isMagic = false, hueVariancePercent = 0, scale = 1): IDie[] {
			...
			if (parenIndex >= 0) {
				...
				if (closeParenIndex >= 0) {
					...
					if (colonIndex >= 0) {
						...
						damageType = this.damageTypeFromStr(damageStr);
						if (damageType === DamageType.Condition || damageType === DamageType.Bane || damageType === DamageType.Bless)
	...
	rollBonusDice() {
		...
		for (let i = 0; i < this.diceRollData.bonusRolls.length; i++) {
			...
			const allDice: IDie[] = this.addDieFromStr(this.diceRollData.diceGroup, bonusRoll.playerID, bonusRoll.diceStr,
				bonusRoll.dieCountsAs, Random.between(1.2, 2.2), 0, bonusRoll.dieBackColor, bonusRoll.dieTextColor, bonusRoll.isMagic, 0, this.diceRollData.overallDieScale);
	...
	reportRollResults(rollResults: RollResults) {
		...
		// TODO: See about using cardDamageMods...
		...
		if (this.totalBonus > 0 && singlePlayerRoll && this.diceRollData.type !== DiceRollType.SkillCheck) {
			this.reportBonusRoll();
		}
	...
	private getDieRollTitle(rollType: DiceRollType) {
		...
		if (rollType === DiceRollType.Initiative)
			title = 'Initiative:';
		else if (rollType === DiceRollType.NonCombatInitiative)
			title = 'Non-combat Initiative:';
		else if (rollType === DiceRollType.SkillCheck)
			title = `${this.getSkillCheckName()} Check:`;
		else if (rollType === DiceRollType.SavingThrow)
	...
	private reportDieTotalsAndMods(nat20s: Map<number, boolean>, singlePlayerId: number, playerIdForTextMessages: number, allModStrs: Array<string>) {
		const totalRoll: number = this.diceRollData.totalRoll;

		const nat20: boolean = nat20s.has(singlePlayerId) && nat20s.get(singlePlayerId);

		if (playerIdForTextMessages !== Creature.invalidCreatureId)
	...
	getCenterPlayerDicePosition(playerName: string): Vector {
		...
		let count = 0;
		this.dice.forEach((die: IDie) => {
			if (die.playerName === playerName && die.inPlay) {
				count++;
				const diePos: Vector = this.getScreenCoordinates(die);
				if (x === undefined) {
					x = diePos.x;
	...
	getFirstCreatureId(diceRollDto: DiceRollData): number {
		...
		return Creature.invalidCreatureId;
	}
	...
	hasMultiPlayerDice(diceRollDto: DiceRollData): boolean {
		...
		for (let i = 0; i < diceRollDto.diceDtos.length; i++) {
			const item: DiceDto = diceRollDto.diceDtos[i];
	...
	highlightSpecialDice() {
		...
		for (let i = 0; i < this.specialDice.length; i++) {
			...
			const scale = thisSpecialDie.scale;
	...
	scaleFallingDice() {
		...
		if (this.scalingDice && this.scalingDice.length > 0) {
			for (let i = 0; i < this.scalingDice.length; i++) {
				...
				const dieScale = thisScalingDie.scale;
	...
	addD100(diceRollData: DiceRollData, backgroundColor: string, textColor: string, playerID: number, throwPower = 1, xPositionModifier = 0, scale = 1) {
		const magicRingHueShift: number = Math.floor(Math.random() * 360);
		// @ts-ignore - DiceD10x10
	...
	addD20(diceGroup: DiceGroup, diceRollData: DiceRollData, d20BackColor: string, d20FontColor: string, xPositionModifier: number, scale = 1, creatureID = Creature.invalidCreatureId): IDie {
		// @ts-ignore - DiceD20
		const die: IDie = new DiceD20({ size: DieRoller.dieScale * scale, backColor: d20BackColor, fontColor: d20FontColor }, this.diceManager);
		die.scale = scale;
	...
	addBadLuckEffects(die: IDie) {
		const sprite: SpriteProxy = diceLayer.addBadLuckRing(die, 960, 540);
		die.attachedSprites.push(sprite);
	...
	rollAddOnDice() {
		...
		if (this.isLuckBent(localDiceRollData)) {
			allDiceAdded = this.addDie(localDiceRollData.diceGroup, 'd4', DamageType.None, DieCountsAs.bentLuck, dieBack, dieFont, throwPower, xPositionModifier, false,
				Creature.invalidCreatureId, '', 0, this.diceRollData.overallDieScale);
			...
			if (localDiceRollData.itsAD20Roll) {   // TODO: Send itsAD20Roll from DM app.
				allDiceAdded.push(this.addD20(localDiceRollData.diceGroup, localDiceRollData, dieBack, dieFont, xPositionModifier, localDiceRollData.overallDieScale));
				allDiceAdded[0].isLucky = true;  // TODO: Send IsLucky from DM app.
			}
			else {
				//let saveDamageModifier: number = damageModifierThisRoll;
				this.addDieFromStr(localDiceRollData.diceGroup, localDiceRollData.playerRollOptions[0].PlayerID, localDiceRollData.damageHealthExtraDice, DieCountsAs.totalScore, throwPower, 0,
					undefined, undefined, false, 0, localDiceRollData.overallDieScale * localDiceRollData.playerRollOptions[0].Scale);
	...
	addD20sForPlayer(playerID: number, xPositionModifier: number, kind: VantageKind, inspiration = '', numD20s = 1, dieLabelOverride: string = null, scale = 1) {
		console.log(`addD20sForPlayer - scale = ${scale}!`);
		...
		for (let i = 0; i < numD20s; i++) {
			const die = this.addD20(this.diceRollData.diceGroup, this.diceRollData, d20BackColor, d20FontColor, xPositionModifier, scale);
			die.scale = scale;
			if (dieLabelOverride)
			...
			if (this.diceRollData.numHalos > 0) {
				...
				for (let j = 0; j < this.diceRollData.numHalos; j++) {
					const sprite: SpriteProxy = diceLayer.addHaloSpin(die, 960, 540, die.scale, diceLayer.activePlayerHueShift + Random.plusMinus(30), angle);
					die.attachedSprites.push(sprite);
		...
		if (inspiration && inspiration !== '-') {
			this.addDieFromStr(this.diceRollData.diceGroup, playerID, inspiration, DieCountsAs.inspiration, this.diceRollData.throwPower, xPositionModifier, d20BackColor, d20FontColor, this.diceRollData.isMagic, 0, this.diceRollData.overallDieScale);
		}
	}
	...
	addDiceFromDto(diceRollDto: DiceRollData, diceDto: DiceDto, xPositionModifier: number) {
		...
		const allDice: IDie[] = this.createDie(diceRollDto.diceGroup, diceDto.Quantity, diceDto.Sides, diceDto.DamageType, diceDto.DieCountsAs, diceDto.BackColor, diceDto.FontColor, this.diceRollData.throwPower, xPositionModifier, diceDto.IsMagic, diceDto.CreatureId, diceDto.Scale * diceRollDto.overallDieScale, '', 0, 0, diceDto.ScoreMultiplier);
		...
		allDice.forEach((die: IDie) => {
			...
			if (diceDto.Label)
				diceLayer.attachLabel(die, diceDto.Label, diceDto.FontColor, diceDto.BackColor, diceDto.Scale * diceRollDto.overallDieScale, this.diceRollData.diceGroup); // So the text matches the die color.
	...
	prepareDiceDtoRoll(diceRollDto: DiceRollData, xPositionModifier: number) {
		...
		//console.log('this.diceRollData.hasMultiPlayerDice (2): ' + this.diceRollData.hasMultiPlayerDice);
	...
	prepareLegacyRoll(xPositionModifier: number) {
		...
		if (this.diceRollData.playerRollOptions.length === 1) {
			playerID = this.diceRollData.playerRollOptions[0].PlayerID;
			scale = this.diceRollData.playerRollOptions[0].Scale;
		}
		...
		if (this.diceRollData.type === DiceRollType.WildMagic) {
			...
			this.addD100(this.diceRollData, diceLayer.activePlayerDieColor, diceLayer.activePlayerDieFontColor, playerID, this.diceRollData.throwPower, xPositionModifier, scale);
		}
			...
			if (this.diceRollData.rollScope === RollScope.ActivePlayer) {
				...
				this.diceRollData.playerRollOptions.forEach((playerRollOption: PlayerRollOptions) => {
					this.addD100(this.diceRollData, diceLayer.activePlayerDieColor, diceLayer.activePlayerDieFontColor, playerRollOption.PlayerID, this.diceRollData.throwPower, xPositionModifier, playerRollOption.Scale);
				});
			...
			this.diceRollData.modifier = 0;
			this.diceRollData.itsAD20Roll = this.diceRollData.type === DiceRollType.DamagePlusSavingThrow || this.diceRollData.type === DiceRollType.OnlyTargetsSavingThrow;
			this.addDieFromStr(this.diceRollData.diceGroup, playerID, this.diceRollData.damageHealthExtraDice, DieCountsAs.damage, this.diceRollData.throwPower, xPositionModifier, diceLayer.activePlayerDieColor, diceLayer.activePlayerDieFontColor, this.diceRollData.isMagic, dieHueVariancePercent, this.diceRollData.overallDieScale);
		}
		else if (this.diceRollData.type === DiceRollType.HealthOnly) {
			this.diceRollData.modifier = 0;
			this.diceRollData.itsAD20Roll = false;
			this.addDieFromStr(this.diceRollData.diceGroup, playerID, this.diceRollData.damageHealthExtraDice, DieCountsAs.health, this.diceRollData.throwPower, xPositionModifier, DiceLayer.healthDieBackgroundColor, DiceLayer.healthDieFontColor, this.diceRollData.isMagic, dieHueVariancePercent, this.diceRollData.overallDieScale);
		}
		else if (this.diceRollData.type === DiceRollType.ExtraOnly) {
			this.diceRollData.modifier = 0;
			this.diceRollData.itsAD20Roll = false;
			this.addDieFromStr(this.diceRollData.diceGroup, playerID, this.diceRollData.damageHealthExtraDice, DieCountsAs.extra, this.diceRollData.throwPower, xPositionModifier, DiceLayer.extraDieBackgroundColor, DiceLayer.extraDieFontColor, this.diceRollData.isMagic, dieHueVariancePercent, this.diceRollData.overallDieScale);
		}
			...
			for (let i = 0; i < this.diceRollData.playerRollOptions.length; i++) {
				...
				this.addD20sForPlayer(playerRollOptions.PlayerID, xPositionModifier, playerRollOptions.VantageKind, playerRollOptions.Inspiration, 0, null, playerRollOptions.Scale);
			}
		}
			...
			this.addDieFromStr(this.diceRollData.diceGroup, playerID, dieStr, DieCountsAs.totalScore, this.diceRollData.throwPower, xPositionModifier, d20BackColor, d20FontColor, false, 0, this.diceRollData.overallDieScale);
		}
		else {
			...
			if (this.diceRollData.rollScope === RollScope.ActivePlayer) {
				...
				this.addD20sForPlayer(playerId, xPositionModifier, vantageKind, this.diceRollData.groupInspiration, numD20s, dieLabel, this.diceRollData.overallDieScale);
				this.diceRollData.hasMultiPlayerDice = false;
			}
			else if (this.diceRollData.rollScope === RollScope.Individuals) {
				this.diceRollData.playerRollOptions.forEach((playerRollOption: PlayerRollOptions) => {
					this.addD20sForPlayer(playerRollOption.PlayerID, xPositionModifier, playerRollOption.VantageKind, playerRollOption.Inspiration, 1, null, playerRollOption.Scale);
				});
				this.diceRollData.hasMultiPlayerDice = this.diceRollData.playerRollOptions.length > 1;
			}
			if (this.isAttack(this.diceRollData)) {
				this.addDieFromStr(this.diceRollData.diceGroup, playerID, this.diceRollData.damageHealthExtraDice, DieCountsAs.damage, this.diceRollData.throwPower, xPositionModifier, diceLayer.activePlayerDieColor, diceLayer.activePlayerDieFontColor, false, dieHueVariancePercent, this.diceRollData.overallDieScale);
			}
		}
		return playerID;
	}
	...
	showSavingThrowTotalsOverAllDice(dice: IDie[], hiddenThreshold: number, diceGroup: DiceGroup) {
		const dieTotals: Map<number, number> = new Map<number, number>();
		dice.forEach((die: IDie) => {
			if (this.countsAsTotalScore(die)) {
				if (!dieTotals.has(die.creatureID)) {
					dieTotals.set(die.creatureID, die.getTopNumber());
				}
				else {
					dieTotals.set(die.creatureID, dieTotals.get(die.creatureID) + die.getTopNumber());
				}
			}
		});
		dieTotals.forEach((total: number, creatureID: number) => {
			this.showSavingThrowTotalsOverDice(this.getCreatureNameFromId(dice, creatureID), dieTotals.get(creatureID), hiddenThreshold, diceGroup);
		});
	}
	...
	showSavingThrowTotalsOverDice(playerName: string, dieTotal: number, hiddenThreshold: number, diceGroup: DiceGroup) {
		...
		let message: string;
		if (dieTotal === DieRoller.Nat20)  // Nat 20!
