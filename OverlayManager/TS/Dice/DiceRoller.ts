...
function updateDieRollSpecialEffects() {
	for (var i = 0; i < dice.length; i++) {
		...
		for (var j = 0; j < diceRollData.trailingEffects.length; j++) {
			let trailingEffect: TrailingEffect = diceRollData.trailingEffects[j];
...
function showRollTotal() {
	...
	if (rollValue >= 0) {
		if (diceRollData.modifier != 0)
			diceLayer.showRollModifier(diceRollData.modifier);
	...
	if (totalDamage > 0) {
		...
		diceLayer.showDamageModifier(damageModifierThisRoll, attemptedRollWasSuccessful);
	}

	if (diceRollData.type != DiceRollType.WildMagic && diceRollData.type != DiceRollType.PercentageRoll) {
