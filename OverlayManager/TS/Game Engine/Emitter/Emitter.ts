class Emitter extends WorldObject {
		...
		try {
			for (var i = 0; i < particlesToCreate; i++) {
				if (this.particlesCreatedSoFar >= this.maxTotalParticles) {
					//console.log('particlesCreatedSoFar >= maxTotalParticles');
					return;
				}
	...
	update(now: number, timeScale: number, world: World): void {
		...
		let particlesToCreate: number = 0;
		if (!this.stopping) {
			particlesToCreate = this.particlesPerSecond * secondsSinceLastParticleCreation * this.percentParticlesToCreate;
			//console.log('particlesToCreate: ' + particlesToCreate);
